...
```bash
git clone <your-repo-url> mahadevmart
cd mahadevmart
...
```bash
mvn compile
mvn exec:java -Dexec.mainClass="com.mahadev.mahadevmart.util.DbSeeder"
```
...
```bash
mvn clean package
cp target/mahadevmart.war $CATALINA_HOME/webapps/
$CATALINA_HOME/bin/startup.sh
```
Visit `http://localhost:8080/mahadevmart/`.
